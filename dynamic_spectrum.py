...
class DynamicSpectrum:
    ...
    def _load_data(self):
        ...
        self.freq = np.load('{}/{}freq.npy'.format(self.ds_path, self.prefix), allow_pickle=True) / 1e6
        self.time = np.load('{}/{}time.npy'.format(self.ds_path, self.prefix), allow_pickle=True) / 3600

        self.XX = np.load('{}/{}dynamic_spectra_XX.npy'.format(self.ds_path, self.prefix), allow_pickle=True) * 1e3
        self.XY = np.load('{}/{}dynamic_spectra_XY.npy'.format(self.ds_path, self.prefix), allow_pickle=True) * 1e3
        self.YX = np.load('{}/{}dynamic_spectra_YX.npy'.format(self.ds_path, self.prefix), allow_pickle=True) * 1e3
        self.YY = np.load('{}/{}dynamic_spectra_YY.npy'.format(self.ds_path, self.prefix), allow_pickle=True) * 1e3
    ...
    def _stack_cal_scans(self):
        ...
        num_scan_intervals = (self.time[self.scan_start_indices[1:]] - self.time[self.scan_end_indices[:-1]]) / avg_scan_dt
        num_channels = self.XX.shape[1]

        # Create initial time-slice to start stacking target and calibrator scans together
        new_data_XX = new_data_XY = new_data_YX = new_data_YY = np.zeros((1, num_channels), dtype=complex)

        for start_index, end_index, num_scans in zip(self.scan_start_indices[:-1],
                                                     self.scan_end_indices[:-1],
                                                     num_scan_intervals):
    ...
    def plot_ds(self, favg, tavg, stokes, cmax=20, save=False, imag=False):
        ...
        if save:
            ...
            fig.savefig(path_template.format(self.ds_path,
                                             self.src.lower(),
                                             stokes.lower(),
                                             fbins,
                                             tbins),
    ...
    def plot_spectrum(self, favg, save=False):
        ...
        if save:
            ...
            sp_fig.savefig(path_template.format(self.ds_path, self.src.lower(), fbins),
                           bbox_inches='tight', format='png', dpi=300)
    ...
    def plot_lightcurve(self, tavg, save=False):
        ...
        if save:
            ...
            lc_fig.savefig(path_template.format(self.ds_path, self.src.lower(), tbins),
                           bbox_inches='tight', format='png', dpi=300)
